...
for b = 0:Nb-1
    ...
    x_b = 2*db * cos(theta_b);    % x-coordinate
    y_b = 2*db * sin(theta_b);    % y-coordinate
...
% Plot the array
figure;
scatter3(Result_antenna_array_positions(:,1), Result_antenna_array_positions(:,2), Result_antenna_array_positions(:,3), 'filled');
xlabel('x [m]');
ylabel('y [m]');
zlabel('z [m]');
