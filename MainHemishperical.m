...
cd '/Users/kafizoa/Dropbox/mMIMO for HAPS/MatlabFiles/FunctionsHemishperical'
% User Parameters
...
antenna_array_positions=compute_hemispherical_array_positions(Nt, z_height,lambda);
...
for group = 1:num_groups
    % Get user positions for this group
     ...
     [H_group,G_group,ThetaTrans_group,RPhiTrans_group] = MIMOHAPSchannel(lambda,D,Theta,Phi,alpha_angle,beta_angle,gamma_angle);
...
%%
Nfig=4;
titleString='SINR heatmap for hemispherical antenna';
%plotHeatMapSINR([user_positions_and_SINR(:,1:3) 10*log10(G(:,194))],R,Nfig,titleString)
plotHeatMapSINR(user_positions_and_SINR,R,Nfig,titleString)
%%
plotCDF(user_positions_and_SINR(:,4)',5,'CDF of SINR for hemispherical antenna','SINR(dB)');
plotCDF(signal_powerAllUsers,6,'CDF of signal power for hemispherical antenna','Power (dB)');
plotCDF(interference_powerAllUsers,7,'CDF of interference power for hemispherical antenna','Power (dB)');
%%
plot_avg_sinr_vs_radius(user_positions_and_SINR, R, 20)
%%
...
% ylabel('Y Position (m)');
% zlabel('Z Position (m)');
% title('Hemispherical Antenna Array Structure in the XY Plane at Z = 20000m');
% axis equal;
% grid on;
%%
% % cd '/Users/kafizoa/Dropbox/mMIMO for HAPS/MatlabFiles/Datasets'
% % save('SINRHemispherical.mat','user_positions_and_SINR')
