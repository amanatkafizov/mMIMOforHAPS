function  [Resultgamma,Resultsignal,Resultinter,Resultnoise] = calculate_sinr(H, B, P_t, P_n)
    ...
    gamma_u = zeros(Nu, 1);
    signal_power= zeros(Nu, 1);
    interference_power= zeros(Nu, 1);
    
    % Loop over each user to calculate SINR
    for u = 1:Nu
        % Desired signal power for user u
        signal_power(u) = abs(H(u, :) * B(:, u))^2 * p_u;
        
        % Interference from all other users i ≠ u
        interference_power(u) = 0;
        for i = 1:Nu
            if i ~= u
                interference_power(u) = interference_power(u) + (abs(H(u, :) * B(:, i))^2) * (P_t / Nu);
            end
        end
